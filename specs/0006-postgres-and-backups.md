...
**Примечание (задним числом, spec 0007):** `scripts/migrate_sqlite_to_postgres.py` и его тест удалены
после того, как схема БД изменилась (переименование `entities`/`entity_items` в `topics`/`concepts`
в spec 0007) — скрипт свою одноразовую задачу уже выполнил, реальные данные пользователя давно в
Postgres, поддерживать его актуальным под новую схему смысла нет.
